...
{
    public class DeveloperCrmClaimsTransformer : IClaimsTransformation
    {
        private readonly ICrmWebApiClient _crmClient;
        private const string UserFields = "domainname,isdisabled";
        private readonly IOptions<DeveloperCrmClaimsTransformOptions> _options;

        public DeveloperCrmClaimsTransformer(ICrmWebApiClient crmClient, IOptions<DeveloperCrmClaimsTransformOptions> options)
        {
        ...
        public async Task<ClaimsPrincipal> TransformAsync(ClaimsPrincipal principal)
        {
            ...
            var crmUser = await FindFirstOrDefaultUser(_options.Value.UserDomainName).ConfigureAwait(false);
        ...
        private async Task<SystemUserDto> FindFirstOrDefaultUser(string domainName)
        {
            ...
            try
            {
                collection = await _crmClient.RetrieveMultipleAsync("systemuser", options, CancellationToken.None).ConfigureAwait(false);
            }
